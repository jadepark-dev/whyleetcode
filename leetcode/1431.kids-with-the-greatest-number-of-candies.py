...
class Solution:
    def kidsWithCandies(self, candies: List[int], extraCandies: int) -> List[bool]:
        maxCandies = max(candies)  # Find the maximum number of candies any child has
        result = [
            candy + extraCandies >= maxCandies for candy in candies
        ]  # List comprehension to check condition for each child
        return result
