...
class Solution(object):
    def cloneGraph(self, node):
        ...
        def dfs(cur):
            # been here, done that
            if cur in map:
                return map[cur]  # get the new node(mapped to old one)
            ...
            for nei in cur.neighbors:
                # copy the neighbor, if it's already in the map, it will be returned
                copied_nei = dfs(nei)
                copy.neighbors.append(copied_nei)

            # return the copied node
            return copy

        # start from the original node(if node is valid)
