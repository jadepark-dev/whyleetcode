...
class Solution(object):
    def canCompleteCircuit(self, gas, cost):
        ...
        total = 0  # keep track of remaining gas amount
        res = 0  # result(starting point)

        for i in range(len(gas)):
            diff = gas[i] - cost[i]
            total += diff

            if total < 0:  # check if the remaining gas can make a tour
                total = 0  # reset the total
                res = i + 1  # move on to the next starting point
